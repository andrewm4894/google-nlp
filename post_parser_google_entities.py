...
where
  ...
  and
  post_id_domain not in (select post_id_domain from nlp.google_entities_all group by 1)
order by 
  post_date_gmt desc
LIMIT
  """+n_posts+"""
...
for index, row in df.iterrows():
    ...
    # try parse entities
    try:
       entities = client.analyze_entity_sentiment(document=document).entities
    except: 
        pass
    ...
    # stream results for each entity into bq
    ...
    if out_df.shape[0] > 0:
        # save results to bq
        pd.io.gbq.to_gbq(out_df,'nlp.google_entities_raw_'+bqsuffix,project_id,if_exists='append')
